        ...
        for index_def in index_definitions:
            try:
                await index_def["collection"].create_index(
                    index_def["keys"], **index_def["options"]
                )
                created_count += 1
                ...
                if "already exists" in error_str or "indexoptionsconflict" in error_str:
                    ...
                    self.logger.exception(f"Error creando índice {index_def['options']['name']}")
                    raise

        self.logger.info(
            f"Índices completados: {created_count} creados, {skipped_count} ya existían"
        )
    ...
    async def setup_ttl_indexes(self):
        ...
        try:
            ...
            await self.db.messages.create_index(
                [("timestamp", 1)], expireAfterSeconds=31536000, name="idx_messages_ttl"
            )  # 365 días

            # TTL para logs de conexion (si se implementa)
            try:
                await self.db.connection_logs.create_index(
                    [("timestamp", 1)], expireAfterSeconds=604800, name="idx_connection_logs_ttl"
                )  # 7 días
            except Exception:
            ...
            await self.db.refresh_tokens.create_index(
                [("expires_at", 1)], expireAfterSeconds=0, name="idx_refresh_tokens_ttl"
            )
            ...
            if "already exists" in error_str or "indexoptionsconflict" in error_str:
                ...
                self.logger.warning(f"No se pudieron configurar indices TTL: {e!s}")
