...
async def protected_route(credentials: HTTPAuthorizationCredentials = Depends(JWTBearer())):
    ...
    try:
        ...
        return {"message": "Acceso autorizado", "user_data": payload}
    except Exception as e:
        auth_logger.error(f"Error en ruta protegida: {e}")
        raise HTTPException(status_code=401, detail="Error al verificar token") from None
...
async def register(user: UserRegister, request: Request):
    ...
    try:
        ...
        if field == "email":
            raise HTTPException(status_code=400, detail="El email ya ha sido registrado") from None
        raise HTTPException(status_code=400, detail="El nombre de usuario ya está en uso") from None
    except Exception as e:
        auth_logger.error(f"Error al insertar usuario: {e}")
        raise HTTPException(status_code=500, detail="Error al registrar usuario") from None
    ...
    return {
        "message": "Usuario registrado correctamente. Por favor, revisa tu correo para confirmar tu cuenta."
    }
...
async def confirm_email(token: str):
    try:
        ...
        if not user:
            ...
            raise HTTPException(
                status_code=400, detail="Token de confirmación inválido o expirado."
            )
...
async def login(user: UserLogin, response: Response):
    try:
        ...
        if not db_user.get("is_email_confirmed"):
            ...
            raise HTTPException(
                status_code=403,
                detail="Email no confirmado. Por favor, revisa tu bandeja de entrada.",
            )
...
async def update_profile(
    ...
    if data.newPassword:
        if not data.currentPassword:
            raise HTTPException(
                status_code=400, detail="Debe indicar la contraseña actual para cambiarla"
            )
...
async def refresh_token_endpoint(
    request: Request, response: Response, body: RefreshTokenRequest | None = Body(None)
):
    """
    ...
    try:
        ...
        is_valid = await refresh_token_service.validate_refresh_token(
            refresh_token_value, user_email
        )
        if not is_valid:
