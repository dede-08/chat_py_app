...
export const chatReducer = (state: ChatState, action: ChatAction): ChatState => {
  switch (action.type) {
    ...
    case CHAT_ACTIONS.SET_USER_TYPING: {
      ...
      if (action.payload.isTyping) newTypingUsers.add(action.payload.userEmail);
      else newTypingUsers.delete(action.payload.userEmail);
      return { ...state, typingUsers: newTypingUsers };
    }

    case CHAT_ACTIONS.SET_USER_ONLINE_STATUS: {
      const newOnlineUsers = new Set(state.onlineUsers);
      if (action.payload.isOnline) newOnlineUsers.add(action.payload.userEmail);
      else newOnlineUsers.delete(action.payload.userEmail);
