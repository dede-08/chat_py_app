...
const RegisterPage = () => {
  ...
  useEffect(() => {
    ...
    const loadRequirements = async () => {
      ...
      try {
        ...
        logger.error('Error al cargar requisitos de contraseña', error instanceof Error ? error : null, { operation: 'getPasswordRequirements' });
  ...
  const validateForm = () => {
    ...
    else if (!isValidEmail(formData.email)) errors.email = 'El email no es válido. Por favor, ingrese un email válido.';
    const usernameValidation = validateUsername(formData.username);
    if (!usernameValidation.isValid && usernameValidation.error) errors.username = usernameValidation.error;
    if (!formData.password) errors.password = 'La contraseña es requerida';
    else if (!passwordValid) errors.password = 'La contraseña no cumple con los requisitos de seguridad';
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      let errorMessage = axiosError.response?.data?.detail || 'Error de conexión. Inténtalo de nuevo.';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <div className="relative">
              ...
              <input type="email" name="email" className={`premium-input pl-11 ${formErrors.email ? 'border-red-500/50 focus:ring-red-500' : ''}`} placeholder="Correo electrónico" value={formData.email} onChange={handleChange} disabled={isSubmitting} />
            </div>
          ...
          <div>
            <div className="relative">
              ...
              <input type={showPassword ? 'text' : 'password'} name="password" className={`premium-input pl-11 pr-11 ${formErrors.password ? 'border-red-500/50 focus:ring-red-500' : ''}`} placeholder="Contraseña" value={formData.password} onChange={handleChange} disabled={isSubmitting} />
          ...
          <div>
            <div className="relative">
              ...
              <input type="text" name="telephone" className={`premium-input pl-11 ${formErrors.telephone ? 'border-red-500/50 focus:ring-red-500' : ''}`} placeholder="Teléfono (ej: +1234567890)" value={formData.telephone} onChange={handleChange} disabled={isSubmitting} />
            </div>
